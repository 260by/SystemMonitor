...
type CPURaw struct {
	...
	Irq     uint64 // time spent servicing  uint64errupts  (since  2.6.0-test4)
...
func getFSInfo(client *ssh.Client, stats *Stats) (err error) {
	lines, err := runCommand(client, "/bin/df -B1|egrep -v 'boot|boot/efi'")
	if err != nil {
		return
	}
	...
	for scanner.Scan() {
		...
		if n == 1 && dev {
			...
			// used, err := strconv.Atoi(parts[2-i])
			if err != nil {
				continue
			}
			free, err := strconv.ParseUint(parts[3-i], 10, 64)
			// free, err := strconv.Atoi(parts[3-i])
			if err != nil {
				continue
			}
...
func getInterfaces(client *ssh.Client, stats *Stats) (err error) {
	var lines string
	lines, err = runCommand(client, "/bin/ip -o addr|egrep -v '127.0.0.1|secondary|docker'")
	if err != nil {
		// try /sbin/ip
		lines, err = runCommand(client, "/sbin/ip -o addr|egrep -v '127.0.0.1|secondary|docker'")
		if err != nil {
			return
		}
	}
	...
	for scanner.Scan() {
		...
		if len(parts) >= 4 && (parts[2] == "inet" || parts[2] == "inet6") {
			...
			if info, ok := stats.NetIntf[intfname]; ok {
				if ipv4 {
					// info.IPv4 = parts[3]
					info.IPv4 = append(info.IPv4, parts[3])
				} else {
					// info.IPv6 = parts[3]
					info.IPv6 = append(info.IPv6, parts[3])
				}
...
func runCommand(client *ssh.Client, cmd string) (stdout string, err error) {
